...
publicationsPerAuthor <- sqldf("SELECT author_lname, author_fname, COUNT(*) FROM PublicationFacts GROUP BY author_fname, author_lname")
publicationsPerJournal <- sqldf("SELECT journal_title, COUNT(DISTINCT pmid) FROM PublicationFacts GROUP BY journal_title")

# Finding Yearly pattern
b <- sqldf("SELECT article_date FROM ArticleFacts")
b<- data.frame(years = substr(b[,], 1,4))
yearly_pattern <- data.frame(years = c("2012", "2013"), stringsAsFactors = F)
yearly_pattern$numOfPubs<-0
yearly_pattern[1, "numOfPubs"] = sqldf("SELECT count(*) FROM b WHERE years LIKE '2012'")
yearly_pattern[2, "numOfPubs"] = sqldf("SELECT count(*) FROM b WHERE years LIKE '2013'")
head(yearly_pattern)
...
```{r}
barplot(seasonal_pattern$numOfPubs, names.arg = seasonal_pattern$quarter, main = "Seasonal Pattern of Publications per quarter", xlab = "Quarters", ylab = "Number of Publications")
barplot(yearly_pattern$numOfPubs, names.arg = yearly_pattern$years, main = " Pattern of Publications per year", xlab = "Years", ylab = "Number of Publications")
```
