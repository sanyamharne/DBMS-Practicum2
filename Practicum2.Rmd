...
abstract_text<-data.frame(xpathSApply(root,xpath,xmlValue))
names(abstract_text)[1]<-"abs_text"
...
for(k in 1:19){
  ...
  if(length(txt)==1){
      abstract_text$label[m]=NA
      abstract_text$nlmcategory[m]=NA
      m=m+1
  }
  else{
    c=1 
    while(c<=length(txt)){
      abstract_text$pmid[m]<-pmid[k]
      abstract_text$label[m]<-label[c]
      abstract_text$nlmcategory[m]<- nlmcategory[c]
...
xpath<-"//History/PubMedPubDate"
pub_date <- xpathSApply(root,xpath,xmlValue)

#Extractingn date in the proper format
...
article_authors<- data.frame(last_name = last_name,
                             fore_name = fore_name,
                             stringsAsFactors = F)
...
xpathEx1<-"//MedlineCitation/PMID"
pmid<-xpathSApply(root,xpathEx1,xmlValue)
article_authors$pmid <- c(1:length(article_authors$last_name))
m = 1
for(k in 1:length(article_authors$last_name)){
  a<-pmid[k]
  xpath<- sprintf("//MedlineCitation[PMID= %s]//Author/LastName", a)
  auth<-xpathSApply(root,xpath,xmlValue)
  c = 1
  while(c<=length(auth)){
    article_authors$pmid[m] <- pmid[k]
    m=m+1
    c=c+1
  }
}
article_authors
inner_join(Author, article_authors, by = c("last_name", "fore_name")) %>% select(pmid, author_id)
```
