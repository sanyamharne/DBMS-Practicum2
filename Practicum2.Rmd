...
## Assumptions : 
1. Created Fact-tables based on the analysis that was required.
2. Considered publications date as the date for data analysis since article date had a lot of inconsistencies.
3. There are publication dates with out any month.
4. Publications made across months are considered to be a part of the prior month for analysis purposes.
5. An inconsistent year "12" in publication date is considered as "2012".
...
```{sql connection=dbcon}
DROP TABLE IF EXISTS ArticleDimension1;
```

```{sql connection=dbcon}
DROP TABLE IF EXISTS JournalDimension1;
```

```{sql connection=dbcon}
DROP TABLE IF EXISTS ArticleDimension2;
```

```{sql connection=dbcon}
DROP TABLE IF EXISTS JournalDimension2;
```

```{sql connection=dbcon}
DROP TABLE IF EXISTS AuthorDimension2;
```
...
## Verifying Status of Tables in database
## Database Status
![Screenshot1](Status1.png)
...
# Part 2: 1. ArticleFacts Star Schema

## 1st FactTable
![ERD](ArticleFacts.png)

## Creating ArticleDimension1 table
```{sql connection=dbcon}
CREATE TABLE IF NOT EXISTS ArticleDimension1(
article_id NUMBER,
pmid NUMBER,
pagination TEXT,
language TEXT,
...
## Creating JournalDimension1 table
```{sql connection=dbcon}
CREATE TABLE IF NOT EXISTS JournalDimension1(
...
article_date TEXT,
article_title TEXt,
FOREIGN KEY(pmid) REFERENCES ArticleDimension1(pmid),
FOREIGN KEY(journal_id) REFERENCES JournalDimension1(journal_id)
);
```

## Verifying Status of Tables in database
## Database Status
![Screenshot2](Status2.png)

## Populating ArticleDimension1 Table
```{r}
...
```{sql connection=dbcon}
SELECT * FROM ArticleDimension1 LIMIT 5;
```

## Populating JournalDimension1 Table
```{r}
...
```{sql connection=dbcon}
SELECT * FROM JournalDimension1 LIMIT 5;
```
...
## Part 2: 2. PublicationFacts Star Schema
![ERD](PublicationFacts.png)

## Creating ArticleDimension2 table
```{sql connection=dbcon}
CREATE TABLE IF NOT EXISTS ArticleDimension2(
article_id NUMBER,
pmid NUMBER,
pagination TEXT,
language TEXT,
...
## Creating JournalDimension2 table
```{sql connection=dbcon}
CREATE TABLE IF NOT EXISTS JournalDimension2(
journal_id NUMBER NOT NULL,
issn TEXT,
issn_type TEXT,
volume NUMBER,
...
## Creating AuthorDimension2 table
```{sql connection=dbcon}
CREATE TABLE IF NOT EXISTS AuthorDimension2(
  author_id NUMBER,
  initials TEXT,
  affiliation TEXT,
  PRIMARY KEY(author_id)
...
author_fname TEXT,
article_title TEXT,
FOREIGN KEY(pmid) REFERENCES ArticleDimension2(pmid),
FOREIGN KEY(journal_id) REFERENCES JournalDimension2(journal_id),
FOREIGN KEY(author_id) REFERENCES AuthorDimension2(author_id)
);
```

## Verifying Status of Tables in database
## Database Status
![Screenshot3](Status3.png)

## Populating ArticleDimension2 Table
```{r}
...
```{sql connection=dbcon}
SELECT * FROM ArticleDimension2 LIMIT 5;
```

## Populating JournalDimension2 Table
```{r}
...
```{sql connection=dbcon}
SELECT * FROM JournalDimension2 LIMIT 5;
```

## Populating AuthorDimension2 Table
```{r}
...
```{sql connection=dbcon}
SELECT * FROM AuthorDimension2 LIMIT 5;
```
